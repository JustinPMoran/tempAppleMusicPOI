...
if (calendarInfoElement) { // Check if the element exists
    ...
    calendarHTML += `<div class="whiteBorderoutline">
        <div class="north tbH1">${monthName}</div>
    </div>
    <div class="calSpacing"></div>`;

    while (j < calendarWeek.length) { // Use calendarWeek.length as the condition
        calendarHTML += `<div class="invertNode northBold">${calendarWeek[j]}</div>`; // Access calendarWeek array elements by index
        j++;
    }
